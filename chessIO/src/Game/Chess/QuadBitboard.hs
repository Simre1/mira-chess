...
newtype SquareData = W4 Word8
              deriving (Bits, Eq, Integral, Ix, Num, Ord, Read, Real, Show)

instance Bounded SquareData where
  minBound = 0
  maxBound = 0xF

instance Enum SquareData where
  succ x | x /= maxBound = x + 1
         | otherwise     = succError "SquareData"
  pred x | x /= minBound = x - 1
         | otherwise     = predError "SquareData"
  toEnum i | i >= 0 && i <= fromIntegral (maxBound::SquareData) = W4 $ fromIntegral i
           | otherwise     = toEnumError "SquareData" i (minBound::SquareData, maxBound::SquareData)
  fromEnum (W4 x) = fromIntegral x
  enumFrom        = boundedEnumFrom
  enumFromThen    = boundedEnumFromThen

instance FiniteBits SquareData where
  finiteBitSize _ = 4
  countLeadingZeros (W4 x) = countLeadingZeros x
  countTrailingZeros (W4 x) = countTrailingZeros x

pattern NoPiece :: SquareData
pattern NoPiece     = 0
...
move :: QuadBitboard -> SquareId -> SquareId -> QuadBitboard
move qbb fromSq toSq = qbb <> move' fromSq (qbb ! fromSq) toSq (qbb ! toSq)

move' :: SquareId -> SquareData -> SquareId -> SquareData -> QuadBitboard
...
enPassant :: SquareId -> SquareId -> QuadBitboard
...
whitePromotion, blackPromotion :: QuadBitboard -> SquareId -> SquareId -> SquareData -> QuadBitboard
...
whitePromotion', blackPromotion' :: SquareId -> SquareId -> SquareData -> SquareData -> QuadBitboard
